...
knitr::opts_chunk$set(fig.width=10, fig.height=25) 
...
- blank correct standards and samples
- calculate standard intensity and chemical ratio
- multiply by standard concentration
...
#add descriptive info back in to samples
...
- still looks like there is machine error?
```{r}
# age breaks
...
```{r}
#try removing outliers and replotting
data <- read.csv(here("Data/Clean/BET_age_breaks_anom_071922.csv"))[-1]%>%
  select(!c("age_diff", "id"))%>%
  filter(element!="Ca")
 ...
 data2 <- data %>% 
  ...
  ungroup() %>% 
  filter(is.na(results))
  percent <- nrow(outliers)/nrow(data)*100
...
SampleID <- unique(data2$ID)

for (i in SampleID) {
  temp_data <- filter(data2, ID==i)
  ...
  plot <- ggplot(data=temp_data, aes(x=Time, y=results))+
    ...
    labs(y="Element ratio", x="")+
    themeo
  #ggsave(plot, path=here("Results/Figures/Outlier removed transects"), file=paste0("plot_", i,".png"), width=10, height=25, units="cm")
}
print(plot)
