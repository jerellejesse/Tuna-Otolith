...
data3 <- filter(data3, Year.Class!=2011)
#end up with 143 samples, but could remove 2013 and 2019 later - ask Lisa
...
data_all <- dplyr::select(data_split, !c("measurement", "std", "conc", "X"))%>%
  spread(key=element, value=results)%>%
  group_by(ID, Year.Capture, Year.Class, Age, Region, Weight, Sex,Age.of.Sample)%>%
...
info_all <- select(data_all, c(1:8))

# try removing the rest of the outliers
datanoout_all <- data_all[,c(9:15)] %>% 
doo(~mahalanobis_distance(.))%>%
   bind_cols(info_all)%>%
   filter(is.outlier == FALSE)
...
datanoout_all_log <- filter(datanoout_all_log, Year.Class!=2011 &Year.Class!= 2019)
...
#stepwise selection
data_step <-as.data.frame(ungroup(datanoout_all_log[,c(14,18:24)]))
